...
- 本目录（`/Users/jiangqianli/Documents/code/health`）默认固定连接老库 `health`；只有用户明确要求时，才临时设置 `DB_NAME=health_new` 连接新库，验收完成后必须改回默认。同级 `health-old` 也固定连接老库 `health`。两个 checkout 独立修改、运行和验收，禁止跨目录混改或通过请求切换数据库。
...
| 项目目录 | 固定数据库 | 用途 |
| --- | --- | --- |
| `/Users/jiangqianli/Documents/code/health`（当前目录） | `health`（老库，默认）；仅用户明确要求时临时切到 `health_new` | 默认口径是老库的模拟器、演示数据和非空数据回归；新库验收只在用户明确要求时进行，且是临时切换，验收完必须改回默认 |
| `/Users/jiangqianli/Documents/code/health-old` | `health` | 老库项目、模拟器、演示数据和非空数据回归 |

配置默认值以 `HealthData/src/main/resources/application.yml` 为准（`DB_NAME` 不设置时默认就是老库 `health`）。数据库密码通过 `DB_PASSWORD` 环境变量提供，不在代码、文档或日志中记录真实凭证。
...
3. 需要新库数据、真实手表接入或新库空态验收时，必须先有用户的明确要求，再临时设置 `DB_NAME=health_new` 启动后端；验收完成后要改回默认（不设置 `DB_NAME`，即老库 `health`）。没有明确要求时不得擅自切换。
4. 两个项目不能同时争用相同的 HTTP、前端、Redis 或 Netty 端口；并行运行时必须为其中一个项目显式分配独立端口和 Redis key 空间。
5. 老库是本项目的默认口径，用于模拟器、演示数据和非空数据回归；涉及数据密度的验收默认要求组织、人员、设备和业务流水存在。只有用户明确要求验收新库时，才允许新库业务数据稀疏或为空。
6. 老库迁移必须显式执行 SQL 脚本，不能在业务请求中自动建表。指挥中心（`/command-center/incidents`）事件表缺失应返回 `503` 并提示部署迁移；Druid SQL 防火墙会拒绝条件 DDL。需要验收新库迁移时，必须先有用户明确要求，再于当前项目临时切换执行，不得默认对新库做变更。

## 指挥中心与事件处置

- 统一管控使用 `/command-center/incidents` 事件模型，事件定位键必须是 `warningId + occurredAt`；分月预警表场景禁止只用裸 `warningId`。
...
前端和后端服务均由用户自行启动和停止（含通过脚本或 IDE），不得代为启动、重启或停止；确认服务状态时用 `curl`/`lsof`/日志核实，而不是自行拉起服务。
...
- 当前项目中的模拟器数据固定写入老库 `health`，用于演示数据和非空回归；模拟器通过 TCP `9000` 发送、后端成功接收并写入 Redis/SQL 的数据，在本项目验收口径中视为真实业务输入。必须区分“数据来源是模拟器”和“数据由前端自行伪造”：前端只能展示后端返回的数据，不得自行生成或替换这些值。真实手表验收只在用户明确要求时，于当前项目临时切到新库进行，并确保模拟器已停止。
- 手表登录后，后端下发 `BP33` 工作模式，并用 `BP86/BP87` 关闭设备内部周期，避免设备和服务端两套调度重叠。后端默认每 60 秒只下发一项测量（可通过 `HEALTH_WATCH_MONITORING_REFRESH_SECONDS` 配置），按 `BPXL -> BPXY -> BPXZ -> BPXT` 轮换，不并发启动传感器，也不把定位 `BP16` 混入健康周期；默认节奏下每项指标约每 4 分钟触发一次，节奏改变时该周期同比例变化。
...
- 实测确认（2026-09-23，姜千里手表 `861265063894429`，全程在线）：`BP40`（下发文字消息）两次下发后手表均未回 `AP40`；同一时段同一台设备的 `BPXY`（测血压）、`BP86`/`BP87`（测量周期设置）、`BP18`（重启）都正常收到确认，说明链路和其他指令都是通的，只有 `BP40` 没反应。也就是说，"发文字"这条指令目前在真实手表上大概率没有实现，不能只凭厂家协议文档认定它有效，页面上不应把"命令已发送"当作"手表已收到"。
- 实测确认（同一批）：`BP28`（语音播报，目前只推送程序合成的提示音，不是真人录音）12 个分包全部发出后，手表对每个分包都回了 `AP28` 确认，说明分包传输链路在当前设备上是通的；但现场没有听到手表出声。也就是说 `AP28` 只证明"分包被设备收到"，不能证明"音频真的播放出来了"——协议里没有专门的"播放完成"反馈。在有可靠的现场听觉验证或设备侧播放反馈之前，语音播报和文字下发都只能算"链路已验证、终端效果未验证"，代码和文档不应把 `AP28`/命令下发成功包装成"用户已收到通知"。

## 测试与验收

当前项目默认只验收老库 `old`，数据密度检查默认要求业务数据非空；登录和配置写入仍必须执行。新库空态验收只在用户明确要求时，于当前项目临时切换后单独进行。
...
5. 改过前端页面后，运行 `node tools/frontend-smoke.cjs --interact`。它用接口登录本地测试账号（令牌放进无头浏览器 Cookie，不在网页里输入密码），逐页检查路由是否到达、控制台报错、HTTP 错误和图表数量，并点击部门图确认能打开抽屉；有任何一项失败退出码为 1。它需要服务已启动，依赖 `HealthShow/package.json` 里声明的 `playwright-core`（在 `HealthShow` 里 `pnpm install`）。前端用 hash 路由，地址必须写成 `/#/health-monitor/...`，用普通路径打开会回退到首页，得到“每页都一样”的假结果。新增或下线页面时同步更新脚本里的路由列表。
6. 停止服务后确认监听端口和项目进程已释放。
...
- 修改数据库连接或兼容路由：当前项目默认只连接老库 `health`，并在当前项目验证老库非空场景；新库变化只在用户明确要求时，于当前项目临时切换处理。检查 Redis key、手表/模拟器路由和权限，不得恢复请求级切库。
