...
5. 改过前端页面后，运行 `node tools/frontend-smoke.cjs --interact`。它用接口登录本地测试账号（令牌放进无头浏览器 Cookie，不在网页里输入密码），逐页检查路由是否到达、控制台报错、HTTP 错误和图表数量，并点击部门图确认能打开抽屉；有任何一项失败退出码为 1。它需要服务已启动，并且需要 `playwright-core`（项目 `package.json` 未声明，脚本不会擅自安装）。前端用 hash 路由，地址必须写成 `/#/health-monitor/...`，用普通路径打开会回退到首页，得到“每页都一样”的假结果。新增或下线页面时同步更新脚本里的路由列表。
