...
6. 老库迁移必须显式执行 SQL 脚本，不能在业务请求中自动建表。指挥中心（`/command-center/incidents`）事件表缺失应返回 `503` 并提示部署迁移；Druid SQL 防火墙会拒绝条件 DDL。需要同时变更新库时，应在同级 `health` 项目中独立执行和验收。
...
- 当前项目中的模拟器数据固定写入老库 `health`，用于演示数据和非空回归；模拟器通过 TCP `9000` 发送、后端成功接收并写入 Redis/SQL 的数据，在本项目验收口径中视为真实业务输入。必须区分“数据来源是模拟器”和“数据由前端自行伪造”：前端只能展示后端返回的数据，不得自行生成或替换这些值。真实手表验收应在同级新库项目中进行，并确保模拟器已停止。
